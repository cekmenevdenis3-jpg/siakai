--[[
================================================================================
    anaclysm hub  v5.2 premium-ui
    Один LocalScript.  StarterPlayer > StarterPlayerScripts
--------------------------------------------------------------------------------
    Меню открывается сразу при запуске.
    Сверху: [—] свернуть (остаётся плавающая кнопка) и [X] полностью закрыть.
...
--==============================================================================
local Theme = {
	-- Obsidian Glass: глубокий графит + холодный violet/teal accent.
	Accent  = Color3.fromRGB(139, 92, 246),
	Accent2 = Color3.fromRGB(45, 212, 191),
	Bg      = Color3.fromRGB(7, 9, 14),
	Bg2     = Color3.fromRGB(11, 14, 22),
	Card    = Color3.fromRGB(17, 21, 31),
	Row     = Color3.fromRGB(22, 27, 39),
	Deep    = Color3.fromRGB(8, 11, 18),
	Stroke  = Color3.fromRGB(48, 56, 76),
	Text    = Color3.fromRGB(245, 247, 255),
	Sub     = Color3.fromRGB(166, 174, 196),
	Dim     = Color3.fromRGB(92, 103, 128),
	Good    = Color3.fromRGB(52, 211, 153),
	Warn    = Color3.fromRGB(251, 191, 36),
	Bad     = Color3.fromRGB(251, 113, 133),
	GlassHi = Color3.fromRGB(31, 37, 53),
	GlassLo = Color3.fromRGB(11, 14, 22),
}
...
do
	...
	TweenService:Create(lblur, TweenInfo.new(0.55, Enum.EasingStyle.Quint), { Size = 18 }):Play()
	...
	New("UIGradient", {
		Color = ColorSequence.new(Color3.fromRGB(4, 6, 11), Color3.fromRGB(11, 9, 19)),
		Rotation = 35, Parent = dim,
	})
	TweenService:Create(dim, TweenInfo.new(0.45), { BackgroundTransparency = 0.06 }):Play()

	local glowA = New("Frame", {
		AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.39, 0.43),
		Size = UDim2.fromOffset(430, 430), BackgroundColor3 = Theme.Accent,
		BackgroundTransparency = 0.91, BorderSizePixel = 0, Parent = gui,
	})
	round1(glowA)
	local glowB = New("Frame", {
		AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.61, 0.57),
		Size = UDim2.fromOffset(360, 360), BackgroundColor3 = Theme.Accent2,
		BackgroundTransparency = 0.94, BorderSizePixel = 0, Parent = gui,
	})
	round1(glowB)
	task.spawn(function()
		TweenService:Create(glowA, TweenInfo.new(3.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
			Position = UDim2.fromScale(0.43, 0.47), BackgroundTransparency = 0.95,
		}):Play()
		TweenService:Create(glowB, TweenInfo.new(4.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
			Position = UDim2.fromScale(0.57, 0.51), BackgroundTransparency = 0.90,
		}):Play()
	end)

	local vp = Camera and Camera.ViewportSize or Vector2.new(900, 600)
	local cardW = math.clamp(vp.X - 36, 338, 520)
	local card = New("CanvasGroup", {
		AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0.5, 18),
		Size = UDim2.fromOffset(cardW, 294), BackgroundColor3 = Theme.Card,
		BackgroundTransparency = 0.04, GroupTransparency = 1, BorderSizePixel = 0, Parent = gui,
	})
	corner(card, 24)
	New("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, Theme.GlassHi),
			ColorSequenceKeypoint.new(0.45, Theme.Card),
			ColorSequenceKeypoint.new(1, Theme.GlassLo),
		}), Rotation = 115, Parent = card,
	})
	local cs = stroke(card, Theme.Stroke, 1, 0.08)
	local csGrad = New("UIGradient", { Color = ColorSequence.new(Theme.Accent, Theme.Accent2), Rotation = 15, Parent = cs })
	csGrad.Transparency = NumberSequence.new({
		NumberSequenceKeypoint.new(0, 0.35), NumberSequenceKeypoint.new(0.5, 0.78), NumberSequenceKeypoint.new(1, 0.35)
	})

	local shine = New("Frame", {
		Position = UDim2.new(0, 22, 0, 1), Size = UDim2.new(1, -44, 0, 1),
	...
	local mark = New("Frame", {
		Position = UDim2.fromOffset(28, 28), Size = UDim2.fromOffset(46, 46),
		BackgroundColor3 = Theme.Accent, BorderSizePixel = 0, Parent = card,
	})
	corner(mark, 15)
	New("UIGradient", { Color = ColorSequence.new(Theme.Accent, Theme.Accent2), Rotation = 45, Parent = mark })
	local core = New("Frame", {
		AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
		Size = UDim2.fromOffset(13, 13), Rotation = 45, BackgroundColor3 = Theme.Bg,
		BorderSizePixel = 0, Parent = mark,
	})
	corner(core, 4)
	task.spawn(function()
		TweenService:Create(core, TweenInfo.new(4.5, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1), { Rotation = 405 }):Play()
	end)

	New("TextLabel", {
		Position = UDim2.fromOffset(88, 27), Size = UDim2.new(1, -190, 0, 23), BackgroundTransparency = 1,
		Font = FB, TextSize = 21, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left,
		Text = "ANACLYSM", Parent = card,
	})
	New("TextLabel", {
		Position = UDim2.fromOffset(89, 51), Size = UDim2.new(1, -190, 0, 16), BackgroundTransparency = 1,
		Font = FM, TextSize = 10, TextColor3 = Theme.Sub, TextXAlignment = Enum.TextXAlignment.Left,
		Text = "PREMIUM INTERFACE  /  v5.2", Parent = card,
	})
	local statePill = New("Frame", {
		AnchorPoint = Vector2.new(1, 0), Position = UDim2.new(1, -28, 0, 31), Size = UDim2.fromOffset(86, 25),
		BackgroundColor3 = Theme.Deep, BackgroundTransparency = 0.12, BorderSizePixel = 0, Parent = card,
	})
	corner(statePill, 9)
	stroke(statePill, Theme.Stroke, 1, 0.25)
	New("Frame", { AnchorPoint=Vector2.new(0,.5), Position=UDim2.new(0,10,.5,0), Size=UDim2.fromOffset(6,6), BackgroundColor3=Theme.Good, BorderSizePixel=0, Parent=statePill })
	local bootLabel = New("TextLabel", {
		Position = UDim2.fromOffset(24, 0), Size = UDim2.new(1, -30, 1, 0), BackgroundTransparency = 1,
		Font = FSB, TextSize = 9, TextColor3 = Theme.Sub, TextXAlignment = Enum.TextXAlignment.Left,
		Text = "BOOTING", Parent = statePill,
	})

	local status = New("TextLabel", {
		Position = UDim2.fromOffset(28, 99), Size = UDim2.new(1, -132, 0, 18), BackgroundTransparency = 1,
	...
	local pct = New("TextLabel", {
		AnchorPoint = Vector2.new(1, 0), Position = UDim2.new(1, -28, 0, 92), Size = UDim2.fromOffset(84, 28),
		BackgroundTransparency = 1, Font = FB, TextSize = 22, TextColor3 = Theme.Text,
		TextXAlignment = Enum.TextXAlignment.Right, Text = "00%", Parent = card,
	})

	local STAGES = {
		{ 34, "CORE" }, { 56, "UI" }, { 78, "MODULES" }, { 94, "PROFILE" },
	}
	local chips = {}
	local stageRow = New("Frame", { Position=UDim2.fromOffset(28,139), Size=UDim2.new(1,-56,0,32), BackgroundTransparency=1, Parent=card })
	for i, st in ipairs(STAGES) do
		local chip = New("Frame", {
			Position = UDim2.new((i - 1) * 0.25, (i - 1) * 3, 0, 0),
			Size = UDim2.new(0.25, -7, 0, 32), BackgroundColor3 = Theme.Deep,
			BackgroundTransparency = 0.18, BorderSizePixel = 0, Parent = stageRow,
		})
		corner(chip, 9)
		local chipStroke = stroke(chip, Theme.Stroke, 1, 0.38)
		local dot = New("Frame", {
			AnchorPoint=Vector2.new(0,.5), Position=UDim2.new(0,9,.5,0), Size=UDim2.fromOffset(6,6),
			BackgroundColor3=Theme.Dim, BorderSizePixel=0, Parent=chip,
		})
		round1(dot)
		local txt = New("TextLabel", {
			Position=UDim2.fromOffset(22,0), Size=UDim2.new(1,-27,1,0), BackgroundTransparency=1,
			Font=FSB, TextSize=8, TextColor3=Theme.Dim, TextXAlignment=Enum.TextXAlignment.Left,
			Text=st[2], Parent=chip,
		})
		chips[i] = { need=st[1], dot=dot, txt=txt, stroke=chipStroke, done=false }
	end

	local track = New("Frame", {
		Position = UDim2.new(0, 28, 0, 198), Size = UDim2.new(1, -56, 0, 6),
		BackgroundColor3 = Theme.Deep, BorderSizePixel = 0, Parent = card,
	})
	round1(track)
	stroke(track, Theme.Stroke, 1, 0.48)
	...
	local glow = New("Frame", {
		AnchorPoint=Vector2.new(1,.5), Position=UDim2.new(1,0,.5,0), Size=UDim2.fromOffset(13,13),
		BackgroundColor3=Theme.Accent2, BackgroundTransparency=.25, BorderSizePixel=0, Parent=fill,
	})
	round1(glow)

	local hint = New("TextLabel", {
		Position = UDim2.fromOffset(28, 224), Size = UDim2.new(1, -56, 0, 18), BackgroundTransparency = 1,
		Font = FM, TextSize = 10, TextColor3 = Theme.Dim, TextXAlignment = Enum.TextXAlignment.Left,
		Text = "Preparing interface modules and restoring your workspace", Parent = card,
	})
	local footer = New("TextLabel", {
		AnchorPoint=Vector2.new(0,1), Position=UDim2.new(0,28,1,-18), Size=UDim2.new(1,-56,0,14), BackgroundTransparency=1,
		Font=FM, TextSize=9, TextColor3=Theme.Dim, TextXAlignment=Enum.TextXAlignment.Left,
		Text="OBSIDIAN GLASS  •  responsive UI  •  adaptive HUD", Parent=card,
	})

	local target, shown, done = 0, 0, false
	TweenService:Create(card, TweenInfo.new(0.52, Enum.EasingStyle.Quint), {
	...
	task.spawn(function()
		while not done and card.Parent do
			...
			if math.abs(target - shown) < 0.25 then shown = target end
			local a = math.clamp(shown / 100, 0, 1)
			fill.Size = UDim2.new(a, 0, 1, 0)
			pct.Text = string.format("%02d%%", math.floor(shown + 0.5))
			glow.Visible = a > 0.015 and a < 0.998
			for _, r in ipairs(chips) do
				if not r.done and shown >= r.need then
					r.done = true
					tw(r.dot, .22, { BackgroundColor3 = Theme.Accent2, Size = UDim2.fromOffset(8,8) }, EASE_SPRING)
					tw(r.txt, .22, { TextColor3 = Theme.Text })
					r.stroke.Color = Theme.Accent
					r.stroke.Transparency = .24
				end
			end
			task.wait(0.025)
		end
	...
	function Loader.finish()
		...
		Loader.set(100, "Интерфейс готов")
		bootLabel.Text = "READY"
		task.wait(0.42)
		done = true
		TweenService:Create(card, TweenInfo.new(0.38, Enum.EasingStyle.Quint), {
			GroupTransparency = 1, Position = UDim2.new(0.5, 0, 0.5, -14),
		}):Play()
		TweenService:Create(glowA, TweenInfo.new(0.38), { BackgroundTransparency = 1 }):Play()
		TweenService:Create(glowB, TweenInfo.new(0.38), { BackgroundTransparency = 1 }):Play()
		TweenService:Create(dim, TweenInfo.new(0.42), { BackgroundTransparency = 1 }):Play()
		TweenService:Create(lblur, TweenInfo.new(0.42), { Size = 0 }):Play()
		task.delay(0.55, function()
			pcall(function() gui:Destroy() end)
			pcall(function() lblur:Destroy() end)
		end)
	end
end
...
local function applySnapshot(s)
	...
	-- Миграция только старой дефолтной пары; пользовательские цвета не трогаем.
	if UICFG.Accent == "#7C5CFF" and UICFG.Accent2 == "#40CCFF" then
		UICFG.Accent, UICFG.Accent2 = "#8B5CF6", "#2DD4BF"
	end
...
local function isEnemy(p)
	...
	-- FFA: у LP нет команды → все враги
	if not LP.Team then return true end
	-- игрок без команды (лобби/наблюдатель) → союзник
...
local function __anaclysm_feature_bootstrap()
	...
	Feature{
		...
		Options = {
			...
			{ Id = "col",   Type = "Color",  Name = "Цвет FOV", Default = "#8B5CF6" },
		},
	...
	Feature{
		...
		Options = {
			{ Id = "learn", Type = "Toggle", Name = "Автообучение по выстрелам", Default = true,
	...
	Feature{
		...
		Options = {
			...
			{ Id = "col",   Type = "Color",  Name = "Цвет", Default = "#2DD4BF" },
	...
	Feature{
		...
		Options = {
			...
			{ Id = "c1", Type = "Color", Name = "Акцент 1", Default = "#8B5CF6" },
			{ Id = "c2", Type = "Color", Name = "Акцент 2", Default = "#2DD4BF" },
		},
	}
	...
	local function __anaclysm_ui_runtime()
		...
		-- Premium visual helpers. They only decorate existing UI and do not touch feature logic.
		function V.surface(obj, radius, transparency)
			if transparency ~= nil then obj.BackgroundTransparency = transparency end
			if not obj:FindFirstChildOfClass("UICorner") then corner(obj, radius or 12) end
			local grad = New("UIGradient", {
				Name = "_premium_gradient", Rotation = 105,
				Color = ColorSequence.new({
					ColorSequenceKeypoint.new(0, Theme.GlassHi),
			...
			local hi = New("Frame", {
				Name = "_premium_highlight", Position = UDim2.new(0, 12, 0, 1), Size = UDim2.new(1, -24, 0, 1),
				BackgroundColor3 = Color3.new(1,1,1), BackgroundTransparency = 0.91, BorderSizePixel = 0,
				ZIndex = math.max(obj.ZIndex + 1, 2), Parent = obj,
			})
			corner(hi, 1)
			return grad
		end

		function V.border(obj, transparency, thickness)
			local st = stroke(obj, Theme.Stroke, thickness or 1, transparency or 0.18)
			st.Name = "_premium_border"
			return st
		end

		function V.accentRail(parent, width)
			local rail = New("Frame", {
				Position = UDim2.new(0, 0, 0, 10), Size = UDim2.new(0, width or 2, 1, -20),
				BackgroundColor3 = Theme.Accent, BorderSizePixel = 0, ZIndex = parent.ZIndex + 2, Parent = parent,
			})
			corner(rail, 2)
			accent(New("UIGradient", { Rotation = 90, Parent = rail }), "Color")
			return rail
		end
		...
		-- watermark ------------------------------------------------------------
		watermark = New("Frame", {
			Name = "watermark", Position = UDim2.new(0, 16, 0, 16), Size = UDim2.fromOffset(10, 34),
			AutomaticSize = Enum.AutomaticSize.X, BackgroundColor3 = Theme.Card, BackgroundTransparency = 0.08,
			BorderSizePixel = 0, ZIndex = 14, Parent = GUI,
		})
		corner(watermark, 12)
		V.surface(watermark, 12, 0.08)
		local wmStroke = V.border(watermark, 0.20, 1)
		padding(watermark, 0, 0, 15, 15)
		local wmBadge = New("Frame", {
			AnchorPoint=Vector2.new(0,.5), Position=UDim2.new(0,-9,.5,0), Size=UDim2.fromOffset(7,7),
			BackgroundColor3=Theme.Accent2, BorderSizePixel=0, ZIndex=16, Parent=watermark,
		})
		round1(wmBadge)
		local wmGlow = New("Frame", {
			AnchorPoint=Vector2.new(.5,.5), Position=UDim2.fromScale(.5,.5), Size=UDim2.fromOffset(17,17),
			BackgroundColor3=Theme.Accent2, BackgroundTransparency=.72, BorderSizePixel=0, ZIndex=15, Parent=wmBadge,
		})
		round1(wmGlow)
		wmText = New("TextLabel", {
			BackgroundTransparency = 1, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = Enum.AutomaticSize.X,
			Font = FSB, TextSize = 11, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left,
			Text = "ANACLYSM", ZIndex=16, Parent = watermark,
		})
		UI.wmBtn = New("TextButton", {
			Name = "wmdrag", BackgroundTransparency = 1, Text = "", AutoButtonColor = false,
			Size = UDim2.new(1, 30, 1, 0), Position = UDim2.fromOffset(-15, 0), ZIndex = 18, Parent = watermark,
		})
		
		
		-- плавающий dock -------------------------------------------------------
		UI.floatDock = New("CanvasGroup", {
			Name = "floating_dock", Size = UDim2.fromOffset(58, 58), BackgroundTransparency = 1,
			GroupTransparency = 0, Visible = false, ZIndex = 30, Parent = GUI,
		})
		
		local function dockButton(parent, y, size, text, primary)
			local f = New("Frame", {
				Position = UDim2.fromOffset((58 - size) / 2, y), Size = UDim2.fromOffset(size, size),
				BackgroundColor3 = Theme.Card, BackgroundTransparency = 0.03,
				BorderSizePixel = 0, ZIndex = 31, Parent = parent,
			})
			corner(f, 16)
			V.surface(f, 16, 0.03)
			local st = V.border(f, 0.08, 1.2)
			local inset = New("Frame", {
				AnchorPoint=Vector2.new(.5,.5), Position=UDim2.fromScale(.5,.5), Size=UDim2.new(1,-10,1,-10),
				BackgroundColor3=Theme.Deep, BackgroundTransparency=.22, BorderSizePixel=0, ZIndex=32, Parent=f,
			})
			corner(inset, 12)
			local ring = stroke(inset, Theme.Accent, 1, .18)
			accent(New("UIGradient", { Rotation=45, Parent=ring }), "Color")
			local l = New("TextLabel", {
				BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Font = FB,
				TextSize = primary and 18 or 15, TextColor3 = Theme.Text, Text = text, ZIndex = 34, Parent = f,
			})
			local b = New("TextButton", {
				BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Text = "", AutoButtonColor = false,
				ZIndex = 35, Parent = f,
			})
			b.MouseEnter:Connect(function()
				tw(inset, 0.14, { BackgroundTransparency = 0.04 })
				tw(st, 0.14, { Transparency = 0 })
			end)
			b.MouseLeave:Connect(function()
				tw(inset, 0.18, { BackgroundTransparency = 0.22 })
				tw(st, 0.18, { Transparency = 0.08 })
			end)
			return b, f
		end
		
		UI.floatMain, UI.floatMainFrame = dockButton(UI.floatDock, 0, 54, "A", true)
		...
		corner(radarPanel, 16)
		V.surface(radarPanel, 16, 0.05)
		UI.radarStroke = V.border(radarPanel, 0.16, 1)
		V.accentRail(radarPanel, 2)
		...
		corner(targetHud, 15)
		V.surface(targetHud, 15, 0.04)
		UI.targetStroke = V.border(targetHud, 0.16, 1)
		V.accentRail(targetHud, 2)
		...
		corner(perfPanel, 15)
		V.surface(perfPanel, 15, 0.04)
		UI.perfStroke = V.border(perfPanel, 0.16, 1)
		V.accentRail(perfPanel, 2)
		...
		corner(threatPanel,15); V.surface(threatPanel,15,.04); UI.threatStroke=V.border(threatPanel,.16,1); V.accentRail(threatPanel,2)
		...
		corner(quickBar,14); V.surface(quickBar,14,.04); UI.quickStroke=V.border(quickBar,.16,1)
		...
		for i=1,8 do
			local b=New("TextButton", { Size=UDim2.fromOffset(30,30), BackgroundColor3=Theme.Deep, BackgroundTransparency=.08, BorderSizePixel=0, AutoButtonColor=false, Font=FSB, TextSize=9, TextColor3=Theme.Sub, Text="—", Visible=false, LayoutOrder=i, ZIndex=18, Parent=UI.quickHolder })
		...
		corner(bindList, 13)
		V.surface(bindList, 13, 0.05)
		V.border(bindList, 0.18, 1)
		...
		function Notify(text, kind)
			...
			corner(f, 13)
			New("UIGradient", { Color=ColorSequence.new(Theme.GlassHi, Theme.GlassLo), Rotation=105, Parent=f })
			local st = V.border(f, 1, 1)
			local bar = New("Frame", {
				Size = UDim2.new(0, 3, 1, -12), Position = UDim2.new(0, 9, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5),
			...
			tw(f, 0.35, { BackgroundTransparency = 0.05, Position = UDim2.new(0, 0, 0, 0) })
		...
		corner(UI.win, 22)
		V.surface(UI.win, 22, 0.02)
		UI.winStroke = V.border(UI.win, 0.05, 1.2)
		local winStrokeGrad = accent(New("UIGradient", { Rotation = 20, Parent = UI.winStroke }), "Color")
		winStrokeGrad.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0,.28), NumberSequenceKeypoint.new(.45,.82), NumberSequenceKeypoint.new(1,.32) })
		
		-- мягкая глубина вместо старого яркого неона
		for i = 1, 3 do
			local g = New("Frame", {
				AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
				Size = UDim2.new(1, i * 10, 1, i * 10), BackgroundColor3 = (i == 1) and Theme.Accent or Color3.new(0,0,0),
				BackgroundTransparency = (i == 1) and 0.965 or (0.80 + i * 0.05), BorderSizePixel = 0, ZIndex = 0, Parent = UI.win,
			})
			corner(g, 22 + i * 5)
			if i == 1 then accent(g, "BackgroundColor3") end
		end
		
		--------------------------------------------------------------- ТОП-БАР ------
		UI.top = New("Frame", {
			Name = "topbar", Size = UDim2.new(1, 0, 0, 50), BackgroundColor3 = Theme.Card, BackgroundTransparency = 0.08,
			BorderSizePixel = 0, ZIndex = 2, Parent = UI.win,
		})
		corner(UI.top, 22)
		New("UIGradient", { Color=ColorSequence.new(Theme.GlassHi, Theme.GlassLo), Rotation=0, Parent=UI.top })
		New("Frame", {
		...
		do
			...
			New("TextLabel", {
				...
				TextXAlignment = Enum.TextXAlignment.Left, Text = "ANACLYSM", ZIndex = 3, Parent = UI.top,
			})
			local chip = New("Frame", {
			...
			New("TextLabel", {
				...
				TextColor3 = Theme.Sub, Text = "v5.2", ZIndex = 4, Parent = chip,
			})
		end
		...
		do
			...
			corner(card, 10)
			V.surface(card, 10, 0.18)
			V.border(card, 0.24, 1)
		...
		UI.side = New("Frame", {
			...
			BackgroundColor3 = Theme.Bg2, BackgroundTransparency = 0.10, BorderSizePixel = 0, Parent = UI.win,
		})
		corner(UI.side, 22)
		New("UIGradient", { Color=ColorSequence.new(Theme.Card, Theme.Bg2), Rotation=90, Parent=UI.side })
		New("Frame", {
		...
		UI.searchBar = New("Frame", {
			...
			BackgroundTransparency = 0.06, BorderSizePixel = 0, Parent = UI.content,
		})
		corner(UI.searchBar, 12)
		V.surface(UI.searchBar, 12, 0.06)
		V.border(UI.searchBar, 0.20, 1)
		...
		local function shell(parent, h)
			local f = New("Frame", {
				Size = UDim2.new(1, 0, 0, h), BackgroundColor3 = Theme.Row, BackgroundTransparency = 0.28,
				BorderSizePixel = 0, Parent = parent,
			})
			corner(f, 11)
			New("UIGradient", { Color=ColorSequence.new(Theme.Row, Theme.Deep), Rotation=90, Parent=f })
			padding(f, 0, 0, 9, 9)
			return f
		end
		...
		local function buildInfoPanel(inner)
			...
			task.spawn(function()
				while inner and inner.Parent do
					...
					rows["Версия"].Text = "anaclysm hub v5.0"
					task.wait(1)
				end
			end)
		end
		...
		local function makeCard(parent, title)
			...
			corner(card, 15)
			V.surface(card, 15, 0.03)
			V.border(card, 0.22, 1)
		...
		local function makeRow(card, f, idx)
			local row = New("Frame", {
				Size = UDim2.new(1, 0, 0, 38), BackgroundColor3 = Theme.Row, BackgroundTransparency = 0.58,
				BorderSizePixel = 0, LayoutOrder = idx * 10, Parent = card,
			})
			corner(row, 10)
			New("UIGradient", { Color=ColorSequence.new(Theme.Row, Theme.Card), Rotation=0, Parent=row })
			...
			local panel = New("ScrollingFrame", {
				Size = UDim2.new(1, 0, 0, 0), BackgroundColor3 = Theme.Deep, BackgroundTransparency = 0.08,
		...
		local function selectTab(id)
			...
			for tid, b in pairs(tabBtns) do
				local on = (tid == id)
				tw(b.f, 0.2, { BackgroundTransparency = on and 0.20 or 1 })
		...
		for i, t in ipairs(TABS) do
			...
			local b = New("TextButton", { BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Text = "", Parent = f })
			b.MouseEnter:Connect(function() if activeTab ~= t.id then tw(f, 0.13, { BackgroundTransparency = 0.68 }) end end)
		...
		function applyUI()
			...
			UI.win.BackgroundTransparency = math.clamp(0.02 + UICFG.Trans * 0.90, 0, 0.48)
			UI.top.BackgroundTransparency = math.clamp(0.08 + UICFG.Trans * 0.55, 0, 0.42)
			UI.side.BackgroundTransparency = math.clamp(0.10 + UICFG.Trans * 0.50, 0, 0.42)
		...
		do
			...
			UI.wmBtn.InputBegan:Connect(function(i)
				if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
					...
					tw(watermark, 0.12, { BackgroundTransparency = 0.15 })
				end
			end)
			bind(UserInputService.InputChanged, function(i)
				if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
			...
			bind(UserInputService.InputEnded, function(i)
				if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
					if dragging then
						tw(watermark, 0.15, { BackgroundTransparency = 0.08 })
						if not moved then
							if menuOpen then
								showMenu(false)
							else
		...
		--============================================= FLOATING DOCK: DRAG / CLICK ===
		do
			local dragging, moved, dStart, pStart = false, false, nil, nil
			UI.floatMain.InputBegan:Connect(function(i)
				if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
					dragging, moved = true, false
					dStart = i.Position
					pStart = Vector2.new(UICFG.FloatX, UICFG.FloatY)
					tw(UI.floatMainFrame, 0.12, { BackgroundTransparency = 0 })
				end
			end)
			bind(UserInputService.InputChanged, function(i)
				if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
					...
					if math.abs(d.X) > 4 or math.abs(d.Y) > 4 then moved = true end
					local vp = VP()
					UICFG.FloatX = math.clamp(pStart.X + d.X, 8, math.max(vp.X - 60, 8))
					UICFG.FloatY = math.clamp(pStart.Y + d.Y, 8, math.max(vp.Y - 60, 8))
					UI.floatDock.Position = UDim2.fromOffset(UICFG.FloatX, UICFG.FloatY)
				end
			end)
			bind(UserInputService.InputEnded, function(i)
				if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
					if dragging then
						tw(UI.floatMainFrame, 0.16, { BackgroundTransparency = 0.05 })
						if not moved then showMenu(true) end
						markDirty()
					end
					dragging = false
				end
		...
		function showMenu(state)
			...
			menuOpen = state
			if UI.floatDock then
				if state then
					UI.floatDock.Visible = false
				else
					UI.floatDock.Visible = true
					UI.floatDock.GroupTransparency = 1
					UI.floatDock.Position = UDim2.fromOffset(UICFG.FloatX + 10, UICFG.FloatY)
					tw(UI.floatDock, 0.24, { GroupTransparency = 0, Position = UDim2.fromOffset(UICFG.FloatX, UICFG.FloatY) })
				end
			end
			animToken += 1
			local tok = animToken
		...
		function hideMenu()
			...
			flushStore()
			showMenu(false)
			Notify("Меню свёрнуто · " .. keyLabel(UICFG.MenuKey) .. " или плавающая кнопка", "ok")
		end
		...
		bind(LP.CharacterAdded, function(char)
			...
			task.wait(0.5)
			for _, f in ipairs(Features) do
				if f._active and f.OnToggle then
					pcall(f.OnToggle, false)
					pcall(f.OnToggle, true)
				end
			end
		end)
		...
		Loader.finish()
		showMenu(true)
		if ById.weaponbridge and O("weaponbridge", "learn") then installSilentHook() end
		...
		task.delay(0.6, function()
			Notify("anaclysm hub v5.2 premium запущен · " .. keyLabel(UICFG.MenuKey), "ok")
			local fallbacks = {}
