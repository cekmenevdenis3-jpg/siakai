--[[
================================================================================
    anaclysm hub  v5.2.2 stability-safe
...
local function __anaclysm_feature_bootstrap()
	...
	local function __anaclysm_ui_runtime()
		...
		if IS_MOBILE then MINW, MINH = 380, 300 end
		...
		do
			...
			New("TextLabel", {
				...
				TextColor3 = Theme.Sub, Text = "v5.2.6", ZIndex = 4, Parent = chip,
			})
		end
		...
		local function buildInfoPanel(inner)
			...
			task.spawn(function()
				while inner and inner.Parent do
					...
					rows["Версия"].Text = "anaclysm hub v5.2.2"
					task.wait(1)
				end
			end)
		end
		...
		local function buildWeaponLabPanel(inner)
			...
			cButton(inner,"Скопировать diagnostics","",function()
				...
				local report=table.concat({
					"anaclysm hub v5.2.2 diagnostics",
		...
		if IS_MOBILE then
			local vp = VP()
			-- Classic compact mobile layout: keep the original two-column menu.
			UICFG.W = math.clamp(math.floor(math.min(UICFG.W, vp.X * 0.60)), MINW, 600)
			UICFG.H = math.clamp(math.floor(math.min(UICFG.H, vp.Y * 0.66)), MINH, 440)
			UICFG.Scale = math.min(UICFG.Scale, 0.75)
			UICFG.X = math.floor(math.max((vp.X - UICFG.W * UICFG.Scale) / 2, 8))
			UICFG.Y = math.floor(math.max((vp.Y - UICFG.H * UICFG.Scale) / 2, 8))
			DB.iface.o.scale = math.floor(UICFG.Scale * 100)
		end
		...
		task.delay(0.6, function()
			Notify("anaclysm hub v5.2.2 запущен · " .. keyLabel(UICFG.MenuKey), "ok")
			local fallbacks = {}
