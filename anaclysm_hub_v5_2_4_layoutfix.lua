--[[
================================================================================
    anaclysm hub  v5.2.4 layout fix + compact compatibility
...
local function gameModeOverride()
	local mode = UICFG.GameModeOverride or "Auto"
	...
	local function __anaclysm_ui_runtime()
		...
		do
			...
			New("TextLabel", {
				...
				TextColor3 = Theme.Sub, Text = "v5.2.4", ZIndex = 4, Parent = chip,
			})
		end
		...
		local function buildInfoPanel(inner)
			...
			local modeBtn
			modeBtn = cButton(inner, "Game mode: " .. gameModeOverride(), "", function()
				local modes = { "Auto", "Teams", "FFA" }
				local cur = gameModeOverride()
				local idx = 1
				for i,v in ipairs(modes) do if v == cur then idx = i break end end
				idx = idx % #modes + 1
				UICFG.GameModeOverride = modes[idx]
				modeBtn.Text = "Game mode: " .. modes[idx]
				markDirty()
				COMPAT_DIAG.lastEvent = "game mode -> " .. modes[idx]
			end)
			cButton(inner, "Очистить compatibility trace", "", function()
			...
			task.spawn(function()
				while inner and inner.Parent do
					...
					local guard = math.max(0, (COMPAT_DIAG.respawnGuardUntil or 0) - os.clock())
					rows["Spawn guard"].Text = guard > 0 and string.format("%.1f s", guard) or "ready"
					rows["Last writer"].Text = COMPAT_DIAG.lastWriter or "—"
					rows["Last change"].Text = COMPAT_DIAG.lastChanges or "—"
					rows["Версия"].Text = "anaclysm hub v5.2.4"
					task.wait(1)
				end
			end)
		end
		...
		local function buildWeaponLabPanel(inner)
			...
			cButton(inner,"Скопировать diagnostics","",function()
				...
				local report=table.concat({
					"anaclysm hub v5.2.4 diagnostics",
		...
		RunService:BindToRenderStep("anaclysm_main", Enum.RenderPriority.Camera.Value + 10, function(dt)
			...
			for _, f in ipairs(Features) do
				...
				if f.Step and f._active then
					local watch = os.clock() < (COMPAT_DIAG.traceUntil or 0)
